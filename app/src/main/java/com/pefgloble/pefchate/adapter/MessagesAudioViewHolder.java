...
public class MessagesAudioViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener, OnProgressListener {
    ...
    public void setUserAudioImage(UsersModel userInfo) {

        if (AppHelper.hasImage(userAudioImage)) return;
        try {
            String ImageUrl = userInfo.getImage();
            ...
            messagesAdapter.glideRequests
                    .asBitmap()
                    .dontAnimate()
                    .load(GlideUrlHeaders.getUrlWithHeaders(EndPoints.ROWS_IMAGE_URL + userInfo.get_id() + "/" + ImageUrl))
